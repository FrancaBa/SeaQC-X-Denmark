...
class StuckValuesDetector(): 
    ...
    def run(self, df_meas_long, measurement_column, time_column, adapted_meas_col_name):
        ...
        is_new_value = (df_meas_long[measurement_column] != df_meas_long[measurement_column].shift()) | df_meas_long[measurement_column].isna()
        groups = is_new_value.cumsum()
        group_sizes = df_meas_long.groupby(groups)[measurement_column].transform('size')
        constant_mask = (group_sizes >= self.window_constant_value) & df_meas_long[measurement_column].notna()

        # Get indices where the mask is True (as check that approach works)
        if constant_mask.any():
            true_indices = constant_mask[constant_mask].index
            self.helper.plot_df(df_meas_long[time_column][true_indices[0]-30:true_indices[0]+50], df_meas_long[measurement_column][true_indices[0]-30:true_indices[0]+50],'Water Level','Timestamp ','Constant period in TS')
            self.helper.plot_df(df_meas_long[time_column][true_indices[-1]-30:true_indices[-1]+50], df_meas_long[measurement_column][true_indices[-1]-30:true_indices[-1]+50],'Water Level','Timestamp ','Constant period in TS (2)')

        # Mask the constant values and add it as a column
        df_meas_long[adapted_meas_col_name] = np.where(constant_mask, np.nan, df_meas_long[measurement_column])
