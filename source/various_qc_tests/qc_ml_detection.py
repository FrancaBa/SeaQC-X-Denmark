...
class MLOutlierDetection(): 
    ...
    def import_data(self, folder_path, tidal_infos):
        ...
        for file in files_only: # Loops through files in the current directory
            ...
            df['original_value'] = df[self.measurement_column].copy()
            df[self.measurement_column] = df[self.measurement_column] - df[self.measurement_column].mean()
            df[self.qc_column] = df[self.qc_column].fillna(0).astype(int)
            #FOR NOW: Allow only a binary QC controll - All QC flags are converted to be the same
            for i in range(len(self.qc_classes) - 1):
                df[self.qc_column] = df[self.qc_column].replace(self.qc_classes[i+1], self.qc_classes[0])
            ...
            self.basic_plotter(df, f'Manual QC for {df_name} station', anomalies = anomaly)
            #Merge dataset according to station name (If station not yet in dict, make a new key with station name and append it, if station already is in dict, append the df to other one from the same station)
            if df_name in self.dfs_merged.keys(): 
                merged_df = pd.concat([self.dfs_merged[f"{df_name}"], df])
                merged_df = merged_df.sort_values(by='Timestamp').reset_index(drop=True)
                self.dfs_merged[f"{df_name}"] = merged_df.copy()
                anomaly = np.isin(self.dfs_merged[f"{df_name}"][self.qc_column], self.qc_classes[0])
                self.basic_plotter(self.dfs_merged[f"{df_name}"], f'Manual QC for {df_name} station combined', anomalies = anomaly)
            else:
                self.dfs_merged[f"{df_name}"] = df.copy()
            #Keep dataset according to unique station (f.e. don't merge Nuuk1 and Nuuk2 as above, but it would merge Nuuk1a and Nuuk1b)
            ...
            if not df[~df[self.qc_column].isin([0, self.qc_classes[0]])].empty:
                raise Exception(f'Code is build for binary classification. QC flags can currently only be good or bad (={self.qc_classes})!')

        return self.dfs_station_subsets, self.dfs_merged
    ...
    def run_training(self, df_dict, combined_training = False, unsupervised_model=False):
        ...
        If combined training is off, the focus station's data is split into train and test data with a ratio of X%. 

        If combined training is on, the different input datasets are split into training and testing data with a ration of X% and merged respectively. The training dataset is also augmented with more fake spikes to balance out class ratio.
        ...
        for elem in df_dict:
            df_train, df_test = train_test_split(df_dict[elem], test_size= self.testing_size, shuffle=False)
            ...
            for value, count in zip(unique_values, counts):
                self.information.append(f"Class {value} exists {count} times in {elem} training dataset. This is {count/len(df_train)}.")
                print(f"Class {value} exists {count} times in {elem} training dataset. This is {count/len(df_train)}.")
                self.information.append('\n')
            anomalies = np.isin(df_train[self.qc_column], [self.qc_classes[0]])
            self.basic_plotter(df_train, f'QC classes (for training data) - Subset {elem}', anomalies = anomalies)
            #Preprocess the training data if needed (for now add some aritifical smaller and larger spikes)
            df_train = self.preprocessing_data(df_train)
            ##Merge data to one combined training and testing dataset if combined training is selected
            if combined_training == True:
                df_train["source"] = elem
        ...
        #Do not run training if model weights are saved
        filepath = os.path.join(self.folder_path, "random_forest.joblib")
        if os.path.isfile(filepath):
            return df_dict, dfs_testing

        #Merge all DataFrames in the dictionary for df_training
        if combined_training == True:
            # Merge all DataFrames in the dictionary for training_df
            df_training = pd.concat(dfs_train.values()).reset_index(drop=True)
            ...
            for value, count in zip(unique_values, counts):
                self.information.append(f"Class {value} exists {count} times in complete training dataset. This is {count/len(df_training)}.")
                print(f"Class {value} exists {count} times in complete training dataset. This is {count/len(df_training)}.")
                self.information.append('\n')

            # Now plot training data set as a whole with different colors for different stations
            plt.figure(figsize=(11, 7))
            for name, group in df_training.groupby("source"):
                plt.plot(group.index, group[self.measurement_column], label=name, marker='o',  markersize=2, linestyle='None')
                anomalies = np.isin(group[self.qc_column], [self.qc_classes[0]])
                if not anomalies.size == 0:
                    plt.scatter(group.index[anomalies], group[self.measurement_column][anomalies], color='blue', facecolors='none', edgecolors='blue', zorder=0.5)
            plt.legend()
            plt.savefig(os.path.join(self.folder_path,f"Combined Training-- Date:{df_training[self.time_column].iloc[0]}.png"), dpi=300, bbox_inches="tight")
            plt.close()

            del df_training["source"]

        #Visualize training data
        anomalies = np.isin(df_training[self.qc_column], [self.qc_classes[0]])
        self.basic_plotter(df_training, 'QC classes (for training data)', anomalies = anomalies)
        #Visualization of station data
        plt.figure(figsize=(11, 7))
        plt.plot(df_training.index, df_training[self.measurement_column], label='Measured Water Level', color='black', marker='o',  markersize=2, linestyle='None')
        if not anomalies.size == 0:
            plt.scatter(df_training.index[anomalies], df_training[self.measurement_column][anomalies], color='blue', label='Manual spike label', facecolors='none', edgecolors='blue', zorder=0.5)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.folder_path,f"TrainingDataSet-- Date:{df_training[self.time_column].iloc[0]}.png"), dpi=300, bbox_inches="tight")
        plt.close() 

        #Save training data inputs and connected outputs for training
        X_train = df_training[self.features].values
        y_train = self.le.fit_transform(np.array(df_training[self.qc_column].values))
        if len(np.unique(y_train)) != 2:
            raise Exception(f'Code is build for binary classification. QC flags can currently only be good or bad, but the input seem to contain more classes!')
    ...
    def run_testing(self, df_dict, dfs_test):
        ...
        """
        #Runs all testing datasets on trained model
        outcomes = {}
        if isinstance(dfs_test, dict):
            #For testing: Use the left-over dfs from all stations which 
            for elem in dfs_test:
                df_outcome = self.eval_testing_model(dfs_test[elem], elem)
                outcomes[elem]= df_outcome
        else:
            #for training on one station: use the testing part of that station's df for testing; Run the complete dfs from the other stations as testing
            for elem in df_dict:
                if self.station in elem:
    ...
    def preprocessing_data(self, df):
        ...
        anomalies = np.isin(df[self.qc_column], [self.qc_classes[0]])
        self.basic_plotter(df, 'Resampled QC classes (small distribution)', anomalies = anomalies)
        ...
        #Visualization resampled data (both steps)
        anomalies = np.isin(df[self.qc_column], [self.qc_classes[0]])
        self.basic_plotter(df, 'Resampled QC classes (large distribution)', anomalies = anomalies)
        #self.zoomable_plot_df(df, 'QC classes - Resampled', anomalies)
    ...
    def add_tidal_signal(self, df, filename, station):
        ...
        ax1.plot(df[self.time_column].values, tide_series, marker='o', markersize=3, color='blue', linestyle='None', alpha=0.9, label = 'TidalSignal')
        ...
        for i in range(0, 20):
            ...
            ax1.plot(df[self.time_column].values[min:max], tide_series[min:max], marker='o', markersize=3, color='blue', linestyle='None', alpha=0.9, label = 'TidalSignal')
            ax1.legend(loc='upper right')
            ax1.xaxis.set_major_formatter(mdates.DateFormatter('%y-%m-%d %H:%M'))
            # Ensure all spines (box edges) are visible
            for spine in ax1.spines.values():
    ...
    def run_model(self, X_train, y_train):
        ...
        #Save the model
        joblib.dump(self.model, os.path.join(self.folder_path, "random_forest.joblib"))
    ...
    def eval_testing_model(self, df_test, station):
        ...
        anomalies = np.isin(df_test[self.qc_column], [self.qc_classes[0]])
        self.basic_plotter(df_test, title, anomalies = anomalies)
        ...
        title = f"ML predicted QC classes (for testing data) -{station}"
        df_test['anomalies_pred'] = np.isin(y_pred, [1])
        self.basic_plotter(df_test, title, anomalies = anomalies, anomalies2 = df_test['anomalies_pred'])
        #self.zoomable_plot_df(df_test, title, anomalies, anomalies_pred)

        #Plot over all periods (weekly plots)
        # Create a 'year-week' column to group by
        df_test['YearWeek'] = df_test[self.time_column].dt.to_period('W')
        # Group by week
        weekly_dfs = {str(f'{week}'): group.drop(columns='YearWeek') for week, group in df_test.groupby('YearWeek')}
        
        for week, df_week in weekly_dfs.items():
            week_number = week.replace('/', '-')
            title = f"ML predicted QC classes (for testing data)- {week_number} - {station}"
            anomalies_weekly = np.isin(df_week[self.qc_column], [self.qc_classes[0]])
            anomalies_weekly_pred = np.isin(df_week['anomalies_pred'], [1])
            self.basic_plotter(df_week, title, anomalies = anomalies_weekly, anomalies2 = anomalies_weekly_pred)

            title = f"ML predicted QC classes (for testing data)2.0- {week_number} - {station}"
            fp = (anomalies_weekly_pred == True) & (anomalies_weekly == False)  # False Positives (FP)
    ...
    def basic_plotter(self, df, title, fn=np.array([]), fp=np.array([]), tp=np.array([]), anomalies=np.array([]), anomalies2=np.array([])):
        """
        ...
        #Visualization of station data
        plt.figure(figsize=(11, 7))
        plt.plot(df['Timestamp'].values, df[self.measurement_column], label='Measured Water Level', color='black', marker='o',  markersize=2, linestyle='None')
        if not anomalies.size == 0:
            plt.scatter(df['Timestamp'][anomalies].values, df[self.measurement_column][anomalies], color='blue', label='Manual spike label', facecolors='none', edgecolors='blue', zorder=0.5)
        if not anomalies2.size == 0:
            plt.scatter(df['Timestamp'][anomalies2].values, df[self.measurement_column][anomalies2], color='red', label='ML spike prediction', zorder=0.2)
        #Add false negatives
        if not fn.size == 0:
    ...
    def save_flagged_series(self, dic_stations):
        """
        Save flagged series from ML QC step to csv.
        """
        for key in dic_stations:
            df = dic_stations[key]
            del df['label']
            #Run model on whole data
            if not self.unsupervised_model:
                y_pred = self.run_test_data(df)
            else:
                raise Exception('Currently, the export function only works for supervised ML training.')

            #Extract relevant columns for export and reset timestamp
            df['quality_flag'] = y_pred
            export_df = df[[self.time_column, 'original_value', self.measurement_column, 'quality_flag']]

            #Save df to csv
            file_name = f"{key}_quality_checked_WL.csv"
